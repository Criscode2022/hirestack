...
@Component({
  ...
  template: `
    <article class="legal">
      ...
      <h1>Privacy</h1>
      <p>HireStack is a two-sided hiring marketplace. We store the account, company, job, application, and message data needed to run that marketplace. Resumes and logos are uploaded to dedicated file storage, not the app server disk.</p>
      ...
      <p>We do not sell candidate lists. We do not sell resumes. Sign-in tokens stay in the browser session. Refresh cookies are httpOnly and limited to authentication routes.</p>
...
@Component({
  ...
  template: `
    <article class="legal">
      ...
      <p>Do not scrape candidate lists, post roles you cannot fill, or store files on our application servers. Uploads go to dedicated file storage only.</p>
